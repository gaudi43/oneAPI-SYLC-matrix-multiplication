...
int gemm(const int M, 
  ...
  std::cout << "\nPerformance Flops = "<<flopsPerMatrixMul<<
    ",\nGPU Computation Time = "<<duration_gpu <<
    "(ms);\nCPU Computaiton Time = "<<duration_cpu<<"(ms);\n";
...
int main() {
  ...
  queue my_gpu_queue(gpu_selector_v , propList);

  int errCode = gemm(1024, 1024, 100, 4, 10, my_gpu_queue);
